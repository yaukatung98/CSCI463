...
BACKUP_BareOS=/path/path/ # The Location where BareOS Puts the Backup Volumes at.

BACKUP_Local=/path/path # The Location where You Would Like to Store the Old Backup Volumes.
...
then
    ...
    then
        ...
        echo "${YELLOW}No House Keeping Volumes Have Passed the 7 Days Retention Period. ${NC}"
        echo "${YELLOW}No House Keeping Volumes Have Passed the 7 Days Retention Period. ${NC}" >> $BACKUP_LOG
    fi
